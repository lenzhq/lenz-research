...
LLMs on Real-World Fact-Checks** — published at
...
This dataset is the **v1.1 harvest** (the cohort behind paper v1.1). The
citable frozen ref is the `llm-disagreement-v1.1` git tag, created when the
paper's served artifacts are frozen. Filenames stay version-free by design:
they are the harness's live input/output paths — tags pin releases, and any
future re-harvest gets its own directory, not renamed files.
...
997-row export (`data.csv`) served from the frozen research snapshot at
[lenz.io/research/llm-disagreement](https://lenz.io/research/llm-disagreement),
not this file. Same lineage, different shape: that file is the derived
per-claim analysis table; this one is the raw per-cell release. To
...
**What ships.** `studies/llm-disagreement/data/` is allow-listed in the repo's
`.gitignore`: only `claims.json`, `results.json`, `results.jsonl`,
`results.csv` and `LICENSE` are tracked. Local working files in that directory
are deliberately excluded — several carry Lenz's
